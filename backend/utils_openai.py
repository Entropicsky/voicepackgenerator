...
def get_image_description(image_base64_data: str, model_name: str) -> Optional[str]:
    ...
    """
    logging.info(f"Attempting to get image description using model: {model_name}")
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        logging.error("OPENAI_API_KEY environment variable not set for image description.")
        return None
    ...
    try:
        ...
        if not image_base64_data.startswith("data:image/") or ";base64," not in image_base64_data:
            logging.error(f"Invalid base64 data URL format: {image_base64_data[:100]}...")
        ...
        logging.debug(f"Sending image to OpenAI model {model_name} for description.")
        ...
        if response.choices and response.choices[0].message and response.choices[0].message.content:
            ...
            logging.info(f"Successfully received image description (len: {len(description)}). Start: {description[:100]}...")
            return description
        else:
            logging.error(f"No content in OpenAI response for image description. Response: {response}")
            return "No description generated by the AI."

    except openai.APIConnectionError as e:
        logging.error(f"OpenAI API request failed to connect during image description: {e}")
        return "Error: Failed to connect to image analysis service."
    except openai.RateLimitError as e:
        logging.error(f"OpenAI API request hit rate limit during image description: {e}")
        return "Error: Image analysis service rate limit exceeded."
    except openai.APIStatusError as e:
        logging.error(f"OpenAI API returned an error status during image description: {e.status_code} - {e.response}")
        return f"Error from image analysis service: {e.message}"
    except Exception as e:
        logging.exception(f"Unexpected error calling OpenAI for image description: {e}")
