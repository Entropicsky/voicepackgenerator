...
def run_script_collaborator_chat_task(self, script_id: int, user_message: str, 
                                     ...
                                     image_base64_data: Optional[str] = None): # NEW: Added image_base64_data
    ...
    try:
        # --- NEW: Image Description Handling --- 
        ...
        if image_base64_data:
            ...
            if image_description and not image_description.startswith("Error:"):
                ...
                # Prepend an error note to the user message so agent is aware
                effective_user_message = f"System Information: Image analysis failed - '{image_description}'.\n\nOriginal user message: {user_message}"
            else: # Should not happen if util returns string, but defensive
        ...
        try:
            user_msg_record = models.ChatMessageHistory(vo_script_id=script_id, role='user', content=user_message) # Log original user message
            # If image was part of user turn, maybe add a placeholder or note in history?
            # For now, just saving the text. The prepended description to agent is for its context, not direct history.
        ...
        proposed_modifications_list = [] # Initialize here
        scratchpad_updates_list = [] # Initialize here
        staged_description_update_result: Optional[StagedCharacterDescriptionData] = None # Initialize here
